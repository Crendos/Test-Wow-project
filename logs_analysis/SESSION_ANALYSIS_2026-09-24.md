...
- **25.09: фикс подтверждён юзером — «благословенный молот работает»** (после повторного запуска `_8.sql`: AreaTriggerId=6006 ✓, но COUNT spline=1 → guard был глобальный; per-Idx доработка = коммит `0134554`, юзер перекачивает файл).
