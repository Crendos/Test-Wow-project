...
### Шаг 4. Боевой лог PalDump (обязательно до следующей сессии)
1. Обнови аддона: `paladin\addons\PalDump` -> `<клиент>\Interface\AddOns\` (замена 2 файлов), `/reload`.
2. Зашёл в мир -> `/paldumplog` (включится, состояние запомнится).
3. Играешь как обычно. Лог сам пишется на диск каждые 10 секунд.
4. Если краш: НЕ убивай клиент задачей, сразу пришли `WTF\Account\<аккаунт>\SavedVariables\PalDump.lua` + новый .dmp.
5. Заодно для следующей партии: в КАЖДОЙ спеке выполни `/paldump` + `/reload` (Свет, Воздаяние, Защита)
