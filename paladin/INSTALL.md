...
Затем: `/paldump` → `/reload` → пришлите мне файл
`<клиент>\WTF\Account\<имя аккаунта>\SavedVariables\PalDump.lua` —
в нём будут ID **всех** способностей книги заклинаний + (если клиент даст) талантов.
