...
Дамп world (26.09) уже разобран. Два следствия, которых в старой инструкции не было:

- `378285` и `469883` в живой базе стоят на `SpellFamilyName=2`. Это ошибка прошлого фикса: паладинское семейство спеллов — **10**, не 2. Пока так, Гнев Тира и Очищающий огонь от Щита мстителя не прокают. Чинится файлом `FIX_FROM_AUDIT.sql` (шаг 1 ниже). Повторно `AUDIT_1_WORLD.sql` / `PROC_FAMILY_FIX.sql` гонять не нужно: они больше не ставят 2.
- Новых биндов (Высшее призвание, Солнечный ожог, Рассвет на Пробуждении, спад кнопки Молота) в базе ещё нет. Они в том же `FIX_FROM_AUDIT.sql`. Старые бинды на месте, E4-семейство у остальных 16 строк (значение 0) трогать не надо.
- Второй файл («это с hotfixes») — только список таблиц, без масок. Маски по-прежнему `MECH_PROC_FIX.sql`.

## 11a. Как поставить ревизию
...
4. В **world** выполнить `FIX_FROM_AUDIT.sql`
   (семейство 10 у 378285/469883 + недостающие бинды). Файлы `_5/_6/_9` повторно
   не обязательны: то, чего не было в дампе, уже собрано в `FIX_FROM_AUDIT.sql`.
5. В **hotfixes** выполнить `MECH_PROC_FIX.sql`
   ...
   Шаг 3 `AUDIT_2` уже прислан: этих четырёх в «проке с чего угодно» нет.
   Старый шаг 1/2 падал на именах колонок — файл исправлен, повторно слать
   1400 строк шага 3 не нужно.
6. Перезапустить мир.
7. `check_all.bat` — строка «механика 26.09 на месте» должна быть зелёной.
