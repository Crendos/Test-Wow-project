...
MainWindow::MainWindow() {
    ...
    // Вкладка «Клиент (копия)» удалена: ASCII-замена в копии вытеснена полноценной
    // вкладкой «Патч WoW.exe» ниже.
    ...
    // Вкладка «QA боты» удалена вместе с её сервисами (qa_*, retail_reference).
    ...
    const int jarvisTabIndex = tabs->addTab(scrollWrap(aiPage), "Джарвис");
    ...
    connect(aiAction,&QAction::triggered,this,[this,tabs,jarvisTabIndex]{tabs->setCurrentIndex(jarvisTabIndex);});
