...
struct Options {
    ...
    // --- явный слот RSA (findslot) ---
    // Файловый offset слота ConnectTo-модуля в ЦЕЛЕВОМ файле: >=0 отключает
    // сигнатурный поиск RSA и пишет ключ точно по адресу (см. huntRsaSlot).
    std::int64_t rsaSlotOffset = -1;
};
...
struct Step {
    ...
    bool mandatory = false;
    std::string note;
    // Явный файловый offset (>=0): писать по адресу, игнорируя сигнатурный
    // поиск. Так работает --rsa-slot после huntRsaSlot.
    std::int64_t forceOffset = -1;
};
...
// ---------------------------------------------------------------------------
// huntRsaSlot (команда findslot): найти слот ConnectTo-модуля в ДРУГОМ билде
// по контексту соседних байтов эталона.
//
// reference — клиент, где слот ИЗВЕСТЕН (обычно уже стоит ключ TrinityCore:
//             это патченый 11.2.5/другой рабочий клиент);
// target    — клиент, у которого слот надо найти (например, чистый 12.1.0);
// maxWindow — окно контекста с каждой стороны (по умолчанию 64; ищем от
//             большего к меньшему, шаг 4, минимум 16).
//
// Контекст берётся вокруг СЛОТА (не вокруг ключа в эталоне), поэтому после
// любого патча эталона соседние байты остаются исходными. Успех = ровно одно
// совпадение обеих сторон контекста; content — 256 байт, лежащие сейчас в
// найденном слоте (это и есть текущий, возможно новый, модуль билда).
// ---------------------------------------------------------------------------
struct SlotHunt {
    bool found = false;
    std::int64_t offset = -1;    // файловый offset слота в target
    int window = 0;              // окно контекста, по которому совпало
    Bytes content;               // 256 байт в найденном слоте target
    std::string refNote;         // как слот выглядел в эталоне
    std::string note;            // содержимое слота / причина отказа
    std::vector<std::string> log;
};
SlotHunt huntRsaSlot(const Bytes &reference, const Bytes &target, int maxWindow = 64);
